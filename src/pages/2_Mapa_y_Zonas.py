...
if columnas_categoricas:
    columna_categoria = st.selectbox(
        "Selecciona la variable para colorear", columnas_categoricas, key="scatter_categorico"
    )
    fig_categoria = scatter_base(
        df,
        color=columna_categoria,
        custom_data=[campo for campo in ["numero", columna_categoria, "kilos_cargados_real"] if campo in df.columns],
...
if col_zona and "kilos_cargados_real" in df.columns:
    ...
    fig_zona.update_traces(
        ...
        hovertemplate=f"Zona: <b>%{{customdata[0]}}</b><br>Kg explosivo: <b>%{{customdata[1]}}" + "</b><extra></extra>",
    )
